...
def generate_ia_realiste(chevaux):
    ...
    for i in range(min(3, len(scores))):
        ...
        if score >= 85:
            ...
            emoji = "🥶"
        pronostic.append(f"{emoji} {cheval} - score ({score})")
    return pronostic

# =====================
# FONCTION PRINCIPALE CONTINUE
# =====================
def run_bot():
    sent = load_sent()
    print("🕒 Bot démarré en continu - Heure Paris :", datetime.now(ZoneInfo("Europe/Paris")).strftime("%H:%M"))
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    # Envoi seulement 10 minutes avant le départ
                    minutes_diff = (heure_course - now).total_seconds() / 60
                    if not (0 <= minutes_diff <= 10):
                        continue
